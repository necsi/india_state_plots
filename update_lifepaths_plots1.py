...
for item in list(pop.keys())[len(list(pop.keys()))-13:]:
    ...
    '''
    numdays=len(np.array([focus.values[-1]]))
    base = datetime.date.today()
    ...
    #print(date_list)
    ax.plot(date_list,[pop[state]/1e6* threshold for x in range(0,len(date_list))],'--', label='1/M', linewidth=2)
